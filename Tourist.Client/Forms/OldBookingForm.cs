...
{
	public partial class OldBookingsForm : MetroForm
	{
		...
		public OldBookingsForm( Form aForm, IRemote aRemote )
		{
			InitializeComponent( );
		...
		private void RoomBookingsDataGrid_CellValidating( object sender, DataGridViewCellValidatingEventArgs e )
		{
			...
			try
			{
				...
				if ( !string.IsNullOrEmpty( RoomBookingsDataGrid[ "R_ClientNifColumn", e.RowIndex ].Value.ToString( ) ) )
				{
					var client =
						Remote.GetClientByNif(
							SharedMethods.ConvertStringToInt( RoomBookingsDataGrid[ "R_ClientNifColumn", e.RowIndex ].Value.ToString( ) ) );
					RoomBookingsDataGrid[ "R_ClientName", e.RowIndex ].Value = client.FirstName + " " + client.LastName;
				}

				if ( !string.IsNullOrEmpty( RoomBookingsDataGrid[ "R_TypeColumn", e.RowIndex ].Value.ToString( ) ) )
				{
					mIdBindingSource.DataSource = Remote.GetBooKablesIds( "Room",
						RoomBookingsDataGrid[ "R_TypeColumn", e.RowIndex ].Value.ToString( ) );
					R_RoomIdColumn.DataSource = mIdBindingSource;
					RoomBookingsDataGrid[ "R_BasePriceColumn", e.RowIndex ].Value =
						Remote.GetBasePrice( RoomBookingsDataGrid[ "R_TypeColumn", e.RowIndex ].Value.ToString( ) )
							.ToString( "0.00", CultureInfo.InvariantCulture );
				}

				if ( !string.IsNullOrEmpty( RoomBookingsDataGrid[ "R_RoomIdColumn", e.RowIndex ].Value.ToString( ) ) )
				{
					RoomBookingsDataGrid[ "R_DescriptionColumn", e.RowIndex ].Value =
						Remote.GetBookableDescription( SharedMethods.ConvertStringToInt(
							RoomBookingsDataGrid[ "R_RoomIdColumn", e.RowIndex ].Value.ToString( ) ),
							RoomBookingsDataGrid[ "R_TypeColumn", e.RowIndex ].Value.ToString( ) );
				}

				if ( !string.IsNullOrEmpty( RoomBookingsDataGrid[ "R_CheckInDateColumn", e.RowIndex ].Value.ToString( ) ) &&
					!string.IsNullOrEmpty( RoomBookingsDataGrid[ "R_CheckOutDateColumn", e.RowIndex ].Value.ToString( ) ) &&
					!string.IsNullOrEmpty( RoomBookingsDataGrid[ "R_TypeColumn", e.RowIndex ].Value.ToString( ) ) )
				{
					var dtr = new DateTimeRange( );

					dtr.StartDateTime =
						SharedMethods.ConvertStringToDateTime( RoomBookingsDataGrid[ "R_CheckInDateColumn", e.RowIndex ].Value.ToString( ) );
					dtr.EndDateTime =
						SharedMethods.ConvertStringToDateTime( RoomBookingsDataGrid[ "R_CheckOutDateColumn", e.RowIndex ].Value.ToString( ) );

					RoomBookingsDataGrid[ "R_TotalPriceColumn", e.RowIndex ].Value =
						( Remote.GetBasePrice( RoomBookingsDataGrid[ "R_TypeColumn", e.RowIndex ].Value.ToString( ) ) *
						 dtr.DiferenceTimeSpan( ).Days ).ToString( "0.00", CultureInfo.InvariantCulture );
				...
				//mIdBindingSource.DataSource = null;
			}
